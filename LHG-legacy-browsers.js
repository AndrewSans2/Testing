...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'Resource/EasyMode/success.mp3', 'path': 'Resource/EasyMode/success.mp3'},
    {'name': 'Resource/EasyMode/no.2.png', 'path': 'Resource/EasyMode/no.2.png'},
    {'name': 'Resource/EasyMode/failed.mp3', 'path': 'Resource/EasyMode/failed.mp3'},
...
async function experimentInit() {
  ...
  psychoJS.experiment.saveAsCSV = function() {};
  psychoJS.experiment.saveAsExcel = function() {};
  ...
  exit_button = new visual.TextStim({
    win: psychoJS.window,
    name: 'exit_button',
    text: 'X',
    font: 'Helvetica',
    units: undefined, 
    pos: [(- 0.97), 0.94], draggable: false, height: 0.15,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('red'),  opacity: undefined,
    depth: -5.0 
  });
  
  toexit_mouse = new core.Mouse({
    win: psychoJS.window,
  });
  toexit_mouse.mouseClock = new util.Clock();
  ...
  filename =
    ...
    "_" + psychoJS._experiment._participant + "_" +
...
function Consent_YesLoopBegin(Consent_YesLoopScheduler, snapshot) {
  return async function() {
    ...
    Consent_Yes.forEach(function() {
      ...
      const tryAgainLoopScheduler = new Scheduler(psychoJS);
      Consent_YesLoopScheduler.add(tryAgainLoopBegin(tryAgainLoopScheduler, snapshot));
      Consent_YesLoopScheduler.add(tryAgainLoopScheduler);
      Consent_YesLoopScheduler.add(tryAgainLoopEnd);
...
var tryAgain;
function tryAgainLoopBegin(tryAgainLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    tryAgain = new TrialHandler({
      psychoJS: psychoJS,
      nReps: tryGateReps, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: undefined,
      seed: undefined, name: 'tryAgain'
    });
    psychoJS.experiment.addLoop(tryAgain); // add the loop to the experiment
    currentLoop = tryAgain;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    tryAgain.forEach(function() {
      snapshot = tryAgain.getSnapshot();
    
      tryAgainLoopScheduler.add(importConditions(snapshot));
      tryAgainLoopScheduler.add(Try_AgainRoutineBegin(snapshot));
      tryAgainLoopScheduler.add(Try_AgainRoutineEachFrame());
      tryAgainLoopScheduler.add(Try_AgainRoutineEnd(snapshot));
      tryAgainLoopScheduler.add(tryAgainLoopEndIteration(tryAgainLoopScheduler, snapshot));
    });
...
async function tryAgainLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(tryAgain);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function tryAgainLoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
function Task1RoutineBegin(snapshot) {
  return async function () {
    ...
    // setup some python lists for storing info about the toexit_mouse
    // current position of the mouse:
    toexit_mouse.x = [];
    toexit_mouse.y = [];
    toexit_mouse.leftButton = [];
    toexit_mouse.midButton = [];
    toexit_mouse.rightButton = [];
    toexit_mouse.time = [];
    toexit_mouse.clicked_name = [];
    gotValidClick = false; // until a click is received
    ...
    Task1Components.push(exit_button);
    Task1Components.push(toexit_mouse);
    
    Task1Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function Task1RoutineEachFrame() {
  return async function () {
    ...
    let mouseClicks = toexit_mouse.getPressed();  // [left, middle, right]
    if (mouseClicks[0] && toexit_mouse.isPressedIn(exit_button)) {
        exitExp = true;           // mark early exit
        trials.finished = true;   // stop loop
        continueRoutine = false;  // end current routine
        psychoJS.eventManager.clearEvents(); // clear lingering clicks/keys
        feedback_type = "";      // to avoid sound in exit
        tryGateReps = 0;
        tryReps = 0;
    }
    ...
    // *toexit_mouse* updates
    if (t >= 0.0 && toexit_mouse.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      toexit_mouse.tStart = t;  // (not accounting for frame time here)
      toexit_mouse.frameNStart = frameN;  // exact frame index
      
      toexit_mouse.status = PsychoJS.Status.STARTED;
      toexit_mouse.mouseClock.reset();
      prevButtonState = toexit_mouse.getPressed();  // if button is down already this ISN'T a new click
    }
    
    // if toexit_mouse is active this frame...
    if (toexit_mouse.status === PsychoJS.Status.STARTED) {
      _mouseButtons = toexit_mouse.getPressed();
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        prevButtonState = _mouseButtons;
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          // check if the mouse was inside our 'clickable' objects
          gotValidClick = false;
          toexit_mouse.clickableObjects = eval(exit_button)
          ;// make sure the mouse's clickable objects are an array
          if (!Array.isArray(toexit_mouse.clickableObjects)) {
              toexit_mouse.clickableObjects = [toexit_mouse.clickableObjects];
          }
          // iterate through clickable objects and check each
          for (const obj of toexit_mouse.clickableObjects) {
              if (obj.contains(toexit_mouse)) {
                  gotValidClick = true;
                  toexit_mouse.clicked_name.push(obj.name);
              }
          }
          if (!gotValidClick) {
              toexit_mouse.clicked_name.push(null);
          }
          _mouseXYs = toexit_mouse.getPos();
          toexit_mouse.x.push(_mouseXYs[0]);
          toexit_mouse.y.push(_mouseXYs[1]);
          toexit_mouse.leftButton.push(_mouseButtons[0]);
          toexit_mouse.midButton.push(_mouseButtons[1]);
          toexit_mouse.rightButton.push(_mouseButtons[2]);
          toexit_mouse.time.push(toexit_mouse.mouseClock.getTime());
        }
      }
    }
    // check for quit (typically the Esc key)
...
function Task1RoutineEnd(snapshot) {
  return async function () {
    ...
    // store data for psychoJS.experiment (ExperimentHandler)
    psychoJS.experiment.addData('toexit_mouse.x', toexit_mouse.x);
    psychoJS.experiment.addData('toexit_mouse.y', toexit_mouse.y);
    psychoJS.experiment.addData('toexit_mouse.leftButton', toexit_mouse.leftButton);
    psychoJS.experiment.addData('toexit_mouse.midButton', toexit_mouse.midButton);
    psychoJS.experiment.addData('toexit_mouse.rightButton', toexit_mouse.rightButton);
    psychoJS.experiment.addData('toexit_mouse.time', toexit_mouse.time);
    psychoJS.experiment.addData('toexit_mouse.clicked_name', toexit_mouse.clicked_name);
...
function FeedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    if (exitExp) {
        ...
        continueRoutine = false;   // skip feedback routine entirely
        exitExp = false;           // reset flag so future trials behave normally
    }
    
    // --- Preload sounds once at the start (e.g., "Begin Experiment" tab) ---
    let correctSound = new Audio("Resource/EasyMode/success.mp3");
    let incorrectSound = new Audio("Resource/EasyMode/failed.mp3");
...
function Try_AgainRoutineBegin(snapshot) {
  return async function () {
    ...
    gotValidClick = false; // until a click is received
    tryReps = 0;
    // --- At the start of the routine (Begin Routine) ---
    gateClock = new util.Clock();  // starts at 0 when routine begins
    ...
    Try_AgainComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function Try_AgainRoutineEnd(snapshot) {
  return async function () {
    ...
    // --- Determine gate choice (End Routine) ---
    let gate_choice = null;
    
    // Check what the participant clicked
    if (try_mouse.clicked_name.includes("try_yes")) {
        gate_choice = 1;  // Yes
        tryReps = 1;       // Start trial loop
        NoReps = 0;
    } 
    else if (try_mouse.clicked_name.includes("try_no")) {
        gate_choice = 0;  // No
        tryReps = 0;       // Skip trial loop
        NoReps = 1;
    }
    
    // --- Record gate to local CSV ---
    psychoJS.experiment.addData('bonus_gate_choice', gate_choice);
    psychoJS.experiment.addData('mouse_time', gateClock.getTime());
    
    // --- Push gate into _trialsData for DataPipe upload ---
    psychoJS._experiment._trialsData.push({
        trial_type: "bonus_gate",
        gate_choice: gate_choice,
        mouse_time: gateClock.getTime()
    });
...
function TestTaskRoutineBegin(snapshot) {
  return async function () {
    ...
    TestTaskComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function NoConsentRoutineBegin(snapshot) {
  return async function () {
    ...
    NoConsentComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
